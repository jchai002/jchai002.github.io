...
$(function(){
	$('.menu_s .m').css({opacity:.7});
	$('.menu ul li').mouseover(function(){
		var x = $(this).offset();
		$(this).find('div').css('top', (x.top + 10)+'px');
		$(this).find('div').css('left', x.left+'px');
		$(this).find('div').show();
	}).mouseout(function(){
		$(this).find('div').hide();
	});
	
	$('#img-play').maiPlayer({speedSwitch: 5000, playerWidth: $(window).width()});


	 $(".top4 .icon .icon_b").mouseover(
                function(){
                $(this).stop().animate({'background-position-y': '-171px'},{duration:500});
      }).mouseout(function(){
      		$(this).stop().animate({'background-position-y': '0px'}, {duration:500});
      });
     
      $(".xinfeng").rotate(
				{bind:{ 
	        		mouseover : function(){ 
						$(this).find('.xinfeng_a').rotate({animateTo:-5});
						$(this).find('.xinfeng_b').stop().animate({'height':'200px', 'top':'30px'}, {duration:500});

	        	},
	       		 	mouseout : function() { 
	            		$(this).find('.xinfeng_a').rotate({animateTo:0});
	            		$(this).find('.xinfeng_b').stop().animate({'height':'422px', 'top':'90px'});
	        	}, 
	        		click : function(){
						var html = $(this).find('.conts').html();
	        			$(this).find('.xinfeng_b').stop().animate({'top':'0px', 'opacity':'0'}, {duration:2000}).hide();
						openNewDiv();
						$('.ele').html(html).show().center();	        			
	        		}
	        	} 
			});
	});
...
function hide(){
	$('.ele').hide('fast', function(){
		$('#mask').remove();
		$('.xinfeng').find('.xinfeng_a').rotate({animateTo:0});
		$('.xinfeng').find('.xinfeng_b').stop().show().animate({'height':'422px', 'top':'90px', 'opacity':'10'});
	});
}
...
function GroupBy(myjson,attr){
    var sum =[];
    $.each(myjson, function(i){
        var temp = {};
        temp[attr] = myjson[i][attr];
        var tempCheck = 0;
        for (var j = 0; j < sum.length; j++) {
            if(sum[j][attr] == temp[attr]){
                tempCheck = 1;
            }
        };
        if(tempCheck == 0){
